 ...
 */
function wpe_graphql_purge_cb( $purge_keys ) {
    $blog_url       = home_url();
    $blog_url_parts = parse_url( $blog_url );
    $hostname       = $blog_url_parts['host'];

    $res = WpeCommon::http_to_varnish( 'PURGE_GRAPHQL', $hostname, [ 
      'GraphQL-Purge-Keys' => $purge_keys,
      'GraphQL-URL' => $hostname, 
    ] );
    if ( is_wp_error( $res ) ) {
        foreach ( $res->get_error_messages() as $error_message ) {
            error_log( "ERROR grapqhl_purge: ". $error_message );
        }
    }
}
add_action( 'graphql_purge', 'wpe_graphql_purge_cb', 0, 1 );
