...
use GravityKit\GravityMath\Foundation\Licenses\WP\WPUpgraderSkin;
use Plugin_Upgrader;
...
class ProductManager {
	...
	public function install_product( array $product ) {
		if ( ! file_exists( ABSPATH . 'wp-admin/includes/class-wp-upgrader.php' ) ) {
			throw new Exception( esc_html__( 'Unable to load core WordPress files required to install the product.', 'gk-gravitymath' ) );
		}
		...
		if ( ! $product_download_link ) {
			throw new Exception( esc_html__( 'Unable to locate product download link.', 'gk-gravitymath' ) );
		}
		...
		try {
			...
			$error = join( ' ', [
				esc_html__( 'Installation failed.', 'gk-gravitymath' ),
				$e->getMessage()
			] );
		...
		if ( ! $installer->result ) {
			throw new Exception( esc_html__( 'Installation failed.', 'gk-gravitymath' ) );
		}
	...
	public function activate_product( $product ) {
		if ( $this->is_product_active_in_current_context( $product ) ) {
			throw new Exception( esc_html__( 'Product is already active.', 'gk-gravitymath' ) );
		}
		...
		if ( is_wp_error( $result ) || ! $this->is_product_active_in_current_context( $product ) ) {
			throw new Exception( esc_html__( 'Could not activate the product.', 'gk-gravitymath' ) );
		}
	...
	public function deactivate_product( $product ) {
		if ( ! $this->is_product_active_in_current_context( $product ) ) {
			throw new Exception( esc_html__( 'Product in not active.', 'gk-gravitymath' ) );
		}

		deactivate_plugins( $product, false, CoreHelpers::is_network_admin() );

		if ( $this->is_product_active_in_current_context( $product ) ) {
			throw new Exception( esc_html__( 'Could not deactivate the product.', 'gk-gravitymath' ) );
		}
	...
	public function update_product( $product_path ) {
		...
		try {
			...
			$error = join( ' ', [
				esc_html__( 'Update failed.', 'gk-gravitymath' ),
	...
	public function get_remote_products() {
		...
		if ( empty( $response ) ) {
			throw new Exception( esc_html__( 'Invalid product information received from the API.', 'gk-gravitymath' ) );
		}
	...
	public function truncate_product_changelog( $changelog, $product_url, $max_changelog_entries = 3, $link_to_full_changelog = true ) {
		...
		if ( count( $parsed_changelog[0] ) > count( $truncated_changelog ) && $link_to_full_changelog ) {
			$truncated_changelog[] = sprintf(
				...
				$product_url,
				esc_html__( 'View full changelog', 'gk-gravitymath' )
			);
		}
	...
	public function display_product_action_links( $links, $product_path ) {
		...
		if ( $product['settings'] ) {
			$gk_links = [
				'settings' => sprintf(
					'<a href="%s">%s</a>',
					$product['settings'],
					esc_html__( 'Settings', 'gk-gravitymath' )
				)
			];
		}
		...
		$gk_links['support'] = sprintf(
			...
			esc_html__( 'Support', 'gk-gravitymath' )
		);
