...
namespace GravityKit\GravityMath\Foundation\Licenses;

use Exception;
use GravityKit\GravityMath\Foundation\Core;
...
class LicenseManager {
	...
	public function ajax_get_licenses_data( array $payload ) {
		if ( ! Framework::get_instance()->current_user_can( 'view_licenses' ) ) {
			throw new Exception( esc_html__( 'You do not have a permission to perform this action.', 'gk-gravitymath' ) );
		}
	...
	public function get_license_key_status_message( $status ) {
		$statuses = [
			'site_inactive'       => esc_html__( 'The license key is valid, but it has not been activated for this site.', 'gk-gravitymath' ),
			'inactive'            => esc_html__( 'The license key is valid, but it has not been activated for this site.', 'gk-gravitymath' ),
			'no_activations_left' => esc_html__( 'This license has reached its activation limit.', 'gk-gravitymath' ),
			'deactivated'         => esc_html__( 'This license has been deactivated.', 'gk-gravitymath' ),
			'valid'               => esc_html__( 'This license key is valid and active.', 'gk-gravitymath' ),
			'invalid'             => esc_html__( 'This license key is invalid.', 'gk-gravitymath' ),
			'missing'             => esc_html__( 'This license key is invalid.', 'gk-gravitymath' ),
			'revoked'             => esc_html__( 'This license key has been revoked.', 'gk-gravitymath' ),
			'expired'             => esc_html__( 'This license key has expired.', 'gk-gravitymath' ),
		];

		if ( empty( $statuses[ $status ] ) ) {
			LoggerFramework::get_instance()->warning( 'Unknown license status: ' . $status );

			return esc_html__( 'License status could not be determined.', 'gk-gravitymath' );
		}
	...
	public function perform_remote_license_call( $license, $edd_action ) {
		...
		foreach ( (array) $response as $key => $data ) {
			if ( ! isset( $data['success'] ) || ! isset( $data['license'] ) || ! isset( $data['checksum'] ) ) {
				throw new Exception( esc_html__( 'License data received from the API is incomplete.', 'gk-gravitymath' ) );
			}
	...
	public function ajax_activate_license( array $payload ) {
		...
		if ( empty( $payload['key'] ) ) {
			throw new Exception( esc_html__( 'Missing license key.', 'gk-gravitymath' ) );
		}
	...
	public function activate_license( $license_key ) {
		...
		if ( isset( $licenses_data[ $license_key ] ) ) {
			throw new Exception( esc_html__( 'This license is already activated.', 'gk-gravitymath' ) );
		}
		...
		try {
			...
			if ( ! $response['_raw']['success'] ) {
				throw new Exception( esc_html__( 'Could not get information on products associated with this license.', 'gk-gravitymath' ) );
			}
		} catch ( Exception $e ) {
			throw new Exception( $e->getMessage() );
		}
	...
	public function ajax_deactivate_license( array $payload ) {
		...
		if ( empty( $licenses_data[ $license_key ] ) ) {
			throw new Exception( esc_html__( 'The license key is invalid.', 'gk-gravitymath' ) );
		}
		...
		try {
			...
			if ( ! $force_removal && ! Arr::get( $response, '_raw.success' ) ) {
				...
				if ( ! Arr::get( $response, 'expiry' ) || ! $this->is_expired_license( Arr::get( $response, 'expiry' ) ) ) {
					throw new Exception( esc_html__( 'Failed to deactivate license.', 'gk-gravitymath' ) );
				}
			}
		} catch ( Exception $e ) {
			if ( ! $force_removal ) {
	...
	public function modify_license_data_for_frontend_output( $license ) {
		...
		if ( ! ctype_alpha( $expiry ) ) {
			...
			$expiry = $expired
				? human_time_diff( $expiry, current_time( 'timestamp' ) ) . ' ' . esc_html_x( 'ago', 'Indicates "time ago"', 'gk-gravitymath' )
	...
	public function display_license_info_on_plugins_page( $plugin_name, $plugin_data ) {
		...
		add_filter( "after_plugin_row_{$plugin_name}", function ( $plugin_name, $plugin_data ) use ( $this_plugin ) {
			...
			$message = strtr(
				esc_html_x( 'This is an unlicensed product. Please [link]visit the licensing page[/link] to enter a valid license or to purchase a new one.', 'Placeholders inside [] are not to be translated.', 'gk-gravitymath' ),
				[
					'[link]'  => '<a href="' . $url . '">',
					'[/link]' => '</a>'
				]
