...
namespace GravityKit\GravityMath\Foundation\Logger;
...
use GravityKit\GravityMath\Foundation\ThirdParty\Monolog\Handler\ChromePHPHandler;
use GravityKit\GravityMath\Foundation\ThirdParty\Monolog\Handler\StreamHandler;
use GravityKit\GravityMath\Foundation\ThirdParty\Monolog\Logger as MonologLogger;
...
use GravityKit\GravityMath\Foundation\ThirdParty\Psr\Log\LoggerTrait;
...
CSS;
		...
		$query_monitor_notice = strtr(
			esc_html_x( 'You must install [link]Query Monitor[/link] WordPress plugin to use this option.', 'Placeholders inside [] are not to be translated.', 'gk-gravitymath' ),
			[
		...
		$chrome_logger_tip = strtr(
			esc_html_x( 'You must install [link]Chrome Logger[/link] browser extension to use this option.', 'Placeholders inside [] are not to be translated.', 'gk-gravitymath' ),
			[
		...
		$gravity_forms_logger_tip = strtr(
			esc_html_x( 'Logging is currently handled by [link]Gravity Forms[/link].', 'Placeholders inside [] are not to be translated.', 'gk-gravitymath' ),
			[
...
HTML;
		...
		$logger_settings = array_merge( $logger_settings, [
			[
				'id'    => 'logger',
				'type'  => 'checkbox',
				'title' => esc_html__( 'Enable Logging', 'gk-gravitymath' ),
				'value' => $logger,
			],
			[
				'id'          => 'logger_type',
				'type'        => 'select',
				'title'       => esc_html__( 'Log Type', 'gk-gravitymath' ),
				'description' => esc_html__( 'Where to store log output', 'gk-gravitymath' ),
				'value'       => $logger_type,
				'choices'     => [
					[
						'title' => esc_html__( 'File', 'gk-gravitymath' ),
						'value' => 'file',
					],
					[
						'title' => esc_html__( 'Query Monitor', 'gk-gravitymath' ),
						'value' => 'query_monitor',
					],
					[
						'title' => esc_html__( 'Chrome Logger', 'gk-gravitymath' ),
						'value' => 'chrome_logger',
					],
				],
				'requires'    => [
		...
		$download_notice = strtr(
			esc_html_x( 'Download [link]log file[/link] ([size] / [date_modified]).', 'Placeholders inside [] are not to be translated.', 'gk-gravitymath' ),
			[
