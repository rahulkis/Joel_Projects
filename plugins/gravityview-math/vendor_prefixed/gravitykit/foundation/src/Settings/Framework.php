...
namespace GravityKit\GravityMath\Foundation\Settings;
...
class Framework {
	...
	public function add_gk_submenu_item() {
		$page_title = $menu_title = esc_html__( 'Settings', 'gk-gravitymath' );
	...
	public function save_ui_settings( array $settings_data ) {
		...
		if ( ! $plugin_id || ! $ui_settings ) {
			throw new Exception( esc_html__( 'Invalid request.', 'gk-gravitymath' ) );
		}
		...
		try {
			...
			if ( empty( $plugin_data['sections'] ) ) {
				throw new ValidatorException( esc_html__( 'Plugin settings data not found.', 'gk-gravitymath' ) );
			}
			...
			foreach ( $plugin_data['sections'] as $section ) {
				...
				foreach ( $section['settings'] as $plugin_setting ) {
					...
					if ( ! empty( $plugin_setting['requires'] ) && is_array( $plugin_setting['requires'] ) && ! $this->are_setting_requirements_met( $plugin_setting, $ui_settings ) ) {
						...
						throw new Exception(
							strtr(
								esc_html_x( 'Setting [setting] has unmet requirements.', 'Placeholders inside [] are not to be translated.', 'gk-gravitymath' ),
			...
			return esc_html__( 'Settings were successfully saved.', 'gk-gravitymath' );
		} catch ( ValidatorException $e ) {
			throw new Exception( sprintf( '%s %s', esc_html__( 'Error saving settings.', 'gk-gravitymath' ), $e->getMessage() ) );
		}
	}
