...
final class Form {
	...
	public function get_auth_screen() {
		...
		$content = array(
			...
			'secured_by_trustedlogin' => '<span class="trustedlogin-logo-large"></span>' . esc_html__( 'Secured by TrustedLogin', 'gk-gravityimport' ),
	...
	private function get_details_html() {
		...
		if ( $this->config->get_setting( 'caps/add' ) || $this->config->get_setting( 'caps/remove' ) ) {
			...
			$roles_summary = sprintf( esc_html__( 'Create a user with a role similar to %s.', 'gk-gravityimport' ), '<strong>' . $cloned_role . '</strong>' );
			$roles_summary .= sprintf( '<small class="tl-' . $ns . '-toggle" data-toggle=".tl-' . $ns . '-auth__role-container">%s <span class="dashicons dashicons--small dashicons-arrow-down-alt2"></span></small>', esc_html__( 'View role capabilities', 'gk-gravityimport' ) );
		} else {
