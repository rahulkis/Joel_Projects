...
if ( ! class_exists( 'Branda_Helper' ) ) {
	...
	class Branda_Helper {
		...
		protected function get_value( $section = null, $name = null, $default = null ) {
			...
			$data = apply_filters( 'ub_escaped_value', $this->esc_deep( $data, $this->esc_callback, $section ), $data, $this->module, $section, $name );
		...
		public function sanitise_input_by_type( $type = null, $value = null, $section_key = null, $key = null, $current_value = null, $data = array() ) {
			...
			switch ( $type ) {
				 ...
				 */
				case 'wp_editor':
					$v                                       = do_shortcode( stripslashes( wp_kses_post( $value[ $section_key ][ $key ] ) ) );
					$v                                       = self::kses_body_markup( $v );
